...
#
# This define creates a pattern configuration file for logstash
#
# == Parameters
#
# [*source*]
...
#
# [*template*]
#   Sets the path to the template to use as content for pattern file
...
define logstash::pattern(
  ...
  file { "logstash_pattern.conf_${name}":
    ensure  => $ensure,
    path    => "${logstash::patterns_dir}/${name}.conf",
    mode    => $logstash::config_file_mode,
    owner   => $logstash::config_file_owner,
    group   => $logstash::config_file_group,
    require => Class['logstash::skel'],
